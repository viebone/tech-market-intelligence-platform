...
| `source` | `str` | Which adapter produced this row: `"greenhouse"`, `"lever"`, `"ashby"`, `"workable"` (added 2026-09-19 — `EMPLOYER_PANEL.md`) (closed set, validated in application code the same way `role_category` is — see Business Logic). `"adzuna"` for legacy rows (backfilled, no longer a live source). |
...
| `job_function` | closed set (see `job-classification.md` — Job Function) \| `"unknown"` \| `None` | **New 2026-09-21.** Only ever non-`None` when `role_category = "other"` — `None` for every Designer/Product Manager/Engineer/`"unknown"` row. Never a fourth `role_category`; a real second-tier breakdown of the `"other"` population instead (Sales & Business Development, Marketing & Communications, ...). See `changes/2026-09-21-taxonomy-revision-specialization-and-job-function.md`. |
| `taxonomy_version` | `str` | The version of `job-classification.md` active when this row was produced. Never rewritten retroactively when the taxonomy changes — see Business Logic — Taxonomy reprocessing (2026-08-11) for how this specific revision is the deliberate exception that proves the rule. Bumped again 2026-09-21 for the specialization widening + Job Function addition. |
