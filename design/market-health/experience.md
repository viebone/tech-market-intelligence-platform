...
| Ask a question in chat (typed or via a suggested-question chip) from any task | The view switches to "Tech market hiring status" (where the conversation lives) and the question is sent there. The chat input is always visible on every task; submitting from a non-conversation task (e.g. "About this platform") opens the conversation rather than silently dropping the answer (added 2026-09-06 — `changes/2026-09-06-chat-input-dead-on-non-conversation-tasks.md`). |
