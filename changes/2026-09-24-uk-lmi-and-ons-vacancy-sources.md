...
- 2026-09-25: **COMMITTED AND DEPLOYED** (PM go: "yes"): commits `96a31bf` (code, 36 files) and `1a9dcc8` (specs/docs/research), pushed to `main`; all five Railway services rebuilt and reached `SUCCESS` (1–2.5 min). Verified against production: story list and welcome shortcuts include `uk-vacancies-official`; `POST /api/market-health/stories/uk-vacancies-official` returns all five sections `ready` (702,000; −8,000; −1.1%; attribution present); the new frontend bundle contains the story; the three admin routes exist (303 to login unauthenticated); OAuth discovery 200. **Not verified:** the MCP tool through a real authenticated client (an unauthenticated POST to `/mcp` returned HTTP 400 — the endpoint responds, but that is not evidence about the tool); the two chart blocks in a real browser.
- 2026-09-25: **Still NOT done:** a scheduled Railway ingestion service (config file committed; needs the dashboard Connect Repo flow and the operator's `STATISTICS_CONTACT`); a browser check of the chart blocks; the Story 5 size-comparison block (experience spec first); `OVERVIEW.md` (now due — Story 5 is user-visible); the country-recording data-quality change request. Next ONS release (expected Oct 2026) needs a manual run of `ingest_trusted_statistics.py` until the service exists.
